...
class EMA():
    ...
    def update_model_average(self, ema_model, current_model):
        for current, ema in zip(current_model.parameters(), ema_model.parameters()):
            old, new = ema.data, current.data
            ema.data = self.update_average(old, new)
...
class GaussianDiffusion(nn.Module):
    ...
    def remove_noise(self, x, t, y, use_ema=True):
        if use_ema:
            return ((x - extract(self.remove_noise_coeff, t, x.shape) * self.ema_model(x, t, y)) * 
    ...
    @torch.no_grad()
    def smaple_diffusion_sequence(self, batch_size, device, y=None, use_ema=True):
        if y is not None and batch_size != len(y):
            raise ValueError("sample batch size different from length of given y")
    ...
    def get_losses(self, x, t, y):
        ...
        perturb_x = self.perturb(x, t, noise)
        
        estimated_noise = self.model(perturb_x, t, y)
